...
# ---------------- config ----------------
SIM_STEP = 1.0                # 1 segundo (modo medio)
HISTORY_SECONDS = 300
...
IA_THRESHOLDS_BASE = {"dtemp_dt": 5.0, "dtorque_dt": 50.0, "dvib_dt": 1.5, "dpower_dt": 10.0, "drpm_dt": 200.0}
...
class Maquina:
    def __init__(self, idx, name):
        ...
        self._state = {"temperature":self.temperature, "torque":self.torque, "vibration":self.vibration,
                       "power":self.power, "rpm":self.rpm}
        ...
        self.history = {"time": deque(maxlen=MAX_HISTORY_LEN),
                        ...
                        "alert": deque(maxlen=MAX_HISTORY_LEN), "severity": deque(maxlen=MAX_HISTORY_LEN)}
    ...
    def inject_variation(self, dtemp=0.0, dtorque=0.0, dvib=0.0, dpower=0.0, drpm=0.0, duration=0.5,
        ...
        self.injected_variation = {"dtemp":dtemp,"dtorque":dtorque,"dvib":dvib,"dpower":dpower,"drpm":drpm,
                                   "remaining":duration, "max_dtemp":max_dtemp,"max_dtorque":max_dtorque,
                                   "max_dvib":max_dvib,"max_dpower":max_dpower,"max_drpm":max_drpm,"reason":reason}
...
class Sistema:
    def __init__(self):
        ...
        self.running = False
        self.log = []
        self.log_columns = ["timestamp","maquina","temperature","torque","vibration","power","rpm","alert","severity","alert_type","recommendation"]
    ...
    def step_all(self, dt):
        with self.lock:
            ...
            for m in self.machines:
                ...
                self.log.append({"timestamp":self.time,"maquina":m.name,"temperature":m.temperature,
                                 "torque":m.torque,"vibration":m.vibration,"power":m.power,"rpm":m.rpm,
                                 "alert":int(last_alert),"severity":sev,"alert_type":atype,"recommendation":rec})
        return alerts
    ...
    def detectar_anomalias(self):
        ...
        for m in self.machines:
            ...
            if random.random() < SPONTANEOUS_FAILURE_PROB_PER_SEC * SIM_STEP:
                m.inject_variation(dtemp=random.uniform(6,20), dtorque=random.uniform(30,180),
                                   dvib=random.uniform(0.6,3.5), dpower=random.uniform(8,30),
                                   drpm=random.uniform(100,600), duration=0.6,
                                   max_dtemp=50, max_dtorque=500, max_dvib=5, max_dpower=80, max_drpm=1000, reason="Spontaneous")
            ...
            if atype:
                ...
                alerts.append({"maquina":m.name, "sector":m.sector, "type":atype, "cause":cause.strip(),
                               "recommendation":rec.strip(), "primary_var":primary_var})
...
class AppGUI:
    ...
    def _build_left_controls(self):
        ...
        self.machine_sel_cb.current(0); self.machine_sel_cb.pack(fill=tk.X, padx=6)
        ...
        ttk.Label(frm, text="ΔT/s:").grid(row=0,column=0, sticky="w"); self.entry_dtemp = tk.DoubleVar(value=6.0); ttk.Entry(frm, textvariable=self.entry_dtemp, width=8).grid(row=0,column=1)
        ttk.Label(frm, text="ΔTorque/s:").grid(row=1,column=0, sticky="w"); self.entry_dtorque = tk.DoubleVar(value=40.0); ttk.Entry(frm, textvariable=self.entry_dtorque, width=8).grid(row=1,column=1)
        ttk.Label(frm, text="ΔVib/s:").grid(row=2,column=0, sticky="w"); self.entry_dvib = tk.DoubleVar(value=1.0); ttk.Entry(frm, textvariable=self.entry_dvib, width=8).grid(row=2,column=1)
        ttk.Label(frm, text="ΔPower/s:").grid(row=3,column=0, sticky="w"); self.entry_dpower = tk.DoubleVar(value=12.0); ttk.Entry(frm, textvariable=self.entry_dpower, width=8).grid(row=3,column=1)
        ttk.Label(frm, text="ΔRPM/s:").grid(row=4,column=0, sticky="w"); self.entry_drpm = tk.DoubleVar(value=200.0); ttk.Entry(frm, textvariable=self.entry_drpm, width=8).grid(row=4,column=1)
        ttk.Label(frm, text="Duración(s):").grid(row=5,column=0, sticky="w"); self.entry_dur = tk.DoubleVar(value=0.6); ttk.Entry(frm, textvariable=self.entry_dur, width=8).grid(row=5,column=1)
    ...
    def _build_center_notebook(self):
        ...
        for i,m in enumerate(self.system.machines):
            ...
            l_temp_val = ttk.Label(read_frame, text="Temp: -- °C", font=("Arial",10))
            l_temp_val.pack(anchor="w", pady=2)
            l_torque_val = ttk.Label(read_frame, text="Torque: -- N·m", font=("Arial",10))
            l_torque_val.pack(anchor="w", pady=2)
            l_vib_val = ttk.Label(read_frame, text="Vibración: -- mm/s", font=("Arial",10))
            l_vib_val.pack(anchor="w", pady=2)
            l_power_val = ttk.Label(read_frame, text="Potencia: -- %", font=("Arial",10))
            l_power_val.pack(anchor="w", pady=2)
            l_rpm_val = ttk.Label(read_frame, text="RPM: --", font=("Arial",10))
            l_rpm_val.pack(anchor="w", pady=2)

            # tolerances panel
            frm_tol = ttk.LabelFrame(read_frame, text="Tolerancias y Temp óptima")
            frm_tol.pack(fill=tk.X, pady=(10,4))
            ttk.Label(frm_tol, text="Temp opt (°C):").grid(row=0,column=0, sticky="w"); temp_opt_var = tk.DoubleVar(value=m.opt_temp); ttk.Entry(frm_tol, textvariable=temp_opt_var, width=8).grid(row=0,column=1)
            ttk.Label(frm_tol, text="Temp % tol:").grid(row=1,column=0, sticky="w"); temp_tol_var = tk.DoubleVar(value=m.tolerances["temperature_pct"]); ttk.Entry(frm_tol, textvariable=temp_tol_var, width=8).grid(row=1,column=1)
            ttk.Label(frm_tol, text="Torque % tol:").grid(row=2,column=0, sticky="w"); torque_tol_var = tk.DoubleVar(value=m.tolerances["torque_pct"]); ttk.Entry(frm_tol, textvariable=torque_tol_var, width=8).grid(row=2,column=1)
            ttk.Label(frm_tol, text="Vib % tol:").grid(row=3,column=0, sticky="w"); vib_tol_var = tk.DoubleVar(value=m.tolerances["vibration_pct"]); ttk.Entry(frm_tol, textvariable=vib_tol_var, width=8).grid(row=3,column=1)
            ttk.Label(frm_tol, text="Power % tol:").grid(row=4,column=0, sticky="w"); power_tol_var = tk.DoubleVar(value=m.tolerances["power_pct"]); ttk.Entry(frm_tol, textvariable=power_tol_var, width=8).grid(row=4,column=1)
            ttk.Label(frm_tol, text="RPM % tol:").grid(row=5,column=0, sticky="w"); rpm_tol_var = tk.DoubleVar(value=m.tolerances["rpm_pct"]); ttk.Entry(frm_tol, textvariable=rpm_tol_var, width=8).grid(row=5,column=1)
            ...
            alert_box = scrolledtext.ScrolledText(read_frame, height=6, state="disabled"); alert_box.pack(fill=tk.X, pady=(0,6))
    ...
    def _build_right_panel(self):
        ...
        self.text_alerts = scrolledtext.ScrolledText(self.right_frame, height=22, state="disabled"); self.text_alerts.pack(fill=tk.BOTH, expand=True, padx=6)
    ...
    def _go_to_selected_machine(self):
        ...
        if 0 <= idx < len(self.system.machines): self.notebook.select(idx)

    def _inject_variation_selected(self):
        idx = self.machine_sel_cb.current()
        if not (0 <= idx < len(self.system.machines)): messagebox.showwarning("Seleccionar","Seleccioná una máquina válida."); return
        dtemp = float(self.entry_dtemp.get()); dtorque = float(self.entry_dtorque.get()); dvib = float(self.entry_dvib.get())
        dpower = float(self.entry_dpower.get()); drpm = float(self.entry_drpm.get()); dur = float(self.entry_dur.get())
        self.system.inject_manual_variation(idx, dtemp=dtemp, dtorque=dtorque, dvib=dvib, dpower=dpower, drpm=drpm, duration=dur,
                                            max_dtemp=abs(dtemp)*2+1, max_dtorque=abs(dtorque)*2+5, max_dvib=max(1.0,abs(dvib)*2),
                                            max_dpower=abs(dpower)*2+5, max_drpm=abs(drpm)*2+50)
    ...
    def _restart_selected_machine(self):
        ...
        if m.active: messagebox.showinfo("Reiniciar",f"{m.name} ya está operativa."); return
        m.repair(); self._restore_visual(idx); messagebox.showinfo("Reiniciar", f"{m.name} reiniciada.")

    def _repair_selected_machine(self):
        idx = self.machine_sel_cb.current()
        if not (0 <= idx < len(self.system.machines)): messagebox.showwarning("Seleccionar","Seleccioná una máquina válida."); return
        m = self.system.machines[idx]
        if m.fault_info is None: messagebox.showinfo("Reparar","No hay falla activa."); return
    ...
    def _handle_alerts_pausing(self, alerts):
        ...
        # summarise first alert
        a0 = alerts[0]
        popup_msg = f"Falla detectada en {a0['maquina']}:\n{a0['type']}\n{a0['cause']}\nRecomendación: {a0['recommendation']}\n\n¿Reparar ahora?"
        ...
        if repair_now:
            ...
            for a in alerts:
                ...
                if idx is not None:
                    with self.system.lock:
                        ...
                        # log repair event
                        self.system.log.append({"timestamp": self.system.time, "maquina": self.system.machines[idx].name,
                                                "temperature": self.system.machines[idx].temperature, "torque": self.system.machines[idx].torque,
                                                "vibration": self.system.machines[idx].vibration, "power": self.system.machines[idx].power,
                                                "rpm": self.system.machines[idx].rpm, "alert":0, "severity":0, "alert_type":"REPAIR", "recommendation":"Reparado manualmente"})
                    self._restore_visual(idx)
            # resume simulation
            self.paused_by_fault = False
            self.start()
            ...
            messagebox.showinfo("Pausado", "Simulación pausada. Repará la falla cuando estés listo.")
            # leave self.paused_by_fault True
    ...
    def _refresh_ui(self):
        ...
        if len(self.system.machines): self.machine_sel_cb.current(0)
